...
function timeSpanToString(startDate, endDate) {
  ...
  let mils = endDate.getMilliseconds() - startDate.getMilliseconds();
  h = h < 10 ? `0${h}` : h;
  m = m < 10 ? `0${m}` : m;
  s = s < 10 ? `0${s}` : s;
  if (mils < 10) mils = `00${mils}`;
...
function angleBetweenClockHands(date) {
  ...
  const angleHours = (hours % 12) * 30 + (minutes / 2);
  const angleMinutes = (minutes % 60) * 6;
  let angle = Math.abs(angleHours - angleMinutes);
  if (angle > 180) {
    angle = 360 - angle;
  }
